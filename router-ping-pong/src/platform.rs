...
use crate::rpc::{Key, Service, TokioSpawner};

pub struct Platform {
    pub outbound: mpsc::Sender<Vec<u8>>,
    pub inbound: Option<mpsc::Receiver<Vec<u8>>>,
    pub status: watch::Sender<PeerStatus>,
    pub rpc: ql_rpc::Router<Key, Service, QlStream, TokioSpawner>,
}
...
impl QlPlatform for Platform {
    ...
    fn persist_peer(&self, _: PeerBundle) {}
    ...
    fn handle_recv_error(&self, error: ReceiveError) {
        eprintln!("rejected QLv2 record: {error:?}");
    }
}
